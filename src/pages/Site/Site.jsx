  ...
  return (
    <section id="galeria" style={{background:'var(--card)',padding:'clamp(60px,8vw,100px) clamp(16px,6vw,80px)'}}>
      ...
      <style>{`
        ...
        .g-loading{position:absolute;inset:0;display:flex;align-items:center;justify-content:center;background:var(--cp-r)}
        .g-spin{width:28px;height:28px;border:3px solid rgba(194,24,91,.25);border-top-color:var(--cp);border-radius:50%;animation:gSpin .8s linear infinite}
...
/* foto de galería con spinner de carga detrás (si no carga, queda el círculo) */
function GalleryImg({ g }) {
  const [loaded, setLoaded] = useState(false)
  return (
    <>
      {!loaded && <div className="g-loading"><span className="g-spin" /></div>}
...
function WhatsAppFAB({ whatsapp, config }) {
  ...
  const [pos, setPos]   = useState(null)   // posicion fijada {side:'left'|'right', top}; null = esquina por defecto
  const [live, setLive] = useState(null)   // posicion en vivo mientras arrastra {left, top}
  ...
  function onUp() {
    ...
    d.active = false
    if (d.moved && live) {
      // siempre pegado a un costado: izquierda o derecha segun el mas cercano (nunca centro)
      const side = (live.left + SIZE / 2) < window.innerWidth / 2 ? 'left' : 'right'
      const p = { side, top: live.top }
      setPos(p); setLive(null)
  ...
  let placement
  if (live) {
    placement = { left: live.left, top: live.top, transition:'none' }
  } else if (pos) {
    const leftPx = pos.side === 'left' ? MARGIN : window.innerWidth - SIZE - MARGIN
    placement = { left: leftPx, top: pos.top, transition:'left .25s ease' }
  } else {
    placement = { right: MARGIN, bottom: MARGIN }
  }
